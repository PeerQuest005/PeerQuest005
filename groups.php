...
$points_stmt = $pdo->prepare("SELECT SUM(points) AS total_possible_points FROM questions_esy_tbl WHERE assessment_id = ?");
...
if (isset($_GET['fetch_chat']) && isset($_GET['room_id'])) {
    ...
    $chat_stmt = $pdo->prepare("
        ...
        WHERE ch.room_id = ?
        ORDER BY ch.time_and_date DESC"  // Change ASC to DESC for reverse order
    );
...
body {
        ...
        .chat-history-scroll ul {
            list-style: none;
            padding: 0;
            margin: 0;
        }
        .chat-history-scroll li {
            padding: 5px 0;
        }
        .chat-user {
            font-weight: bold;
...
<body>
    ...
    <button class="go-back-btn" data-url="view_assessment_teacher.php?class_id=<?php echo htmlspecialchars($class_id); ?>">Go Back to Assessments</button>
    ...
    <?php foreach ($rooms as $room): ?>
        ...
        // Fetch total grades for this room and assessment
        $grades_stmt = $pdo->prepare("
            SELECT SUM(grades) AS total_grades 
            FROM answers_esy_collab_tbl 
        ...
        // Calculate adjusted total points
        $adjusted_total_points = $total_possible_points * $num_students;
        ?>

        <div class="container">
            <div class="room-header">
            <h2>Room ID: <span class="underline"><?php echo htmlspecialchars($room_id); ?></span></h2>
                <a href="grade_groups.php?assessment_id=<?php echo urlencode($assessment_id); ?>&room_id=<?php echo urlencode($room_id); ?>" class="button-link">
                    <button>Grade Group</button>
                </a>
            </div>
